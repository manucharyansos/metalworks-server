...
class AdminOperationsController extends Controller
{
    ...
    public function dashboard(Request $request): JsonResponse
    {
        try {
            ...
            $orderSummary = DB::table('orders as o')
                ...
                ->selectRaw(
                    'COUNT(DISTINCT CASE WHEN (o.status IS NULL OR o.status NOT IN (?, ?, ?)) THEN o.id END) as active_orders',
                    self::CLOSED_ORDER_STATUSES
                )
                ->selectRaw("COUNT(DISTINCT CASE WHEN o.status = 'completed' THEN o.id END) as completed_orders")
                ->selectRaw("COUNT(DISTINCT CASE WHEN o.status IN ('canceled', 'cancelled') THEN o.id END) as canceled_orders")
                ->selectRaw(
                    'COUNT(DISTINCT CASE WHEN (o.status IS NULL OR o.status NOT IN (?, ?, ?)) AND d.finish_date IS NOT NULL AND d.finish_date < ? THEN o.id END) as overdue_orders',
                    [...self::CLOSED_ORDER_STATUSES, $now]
                )
                ->selectRaw(
                    'COUNT(DISTINCT CASE WHEN (o.status IS NULL OR o.status NOT IN (?, ?, ?)) AND d.finish_date >= ? AND d.finish_date < ? THEN o.id END) as due_today',
                    [...self::CLOSED_ORDER_STATUSES, $today, $tomorrow]
                )
                ->selectRaw(
                    'COUNT(DISTINCT CASE WHEN (o.status IS NULL OR o.status NOT IN (?, ?, ?)) AND d.finish_date >= ? AND d.finish_date <= ? THEN o.id END) as due_next_7_days',
                    [...self::CLOSED_ORDER_STATUSES, $tomorrow, $weekEnd]
                )
                ->selectRaw(
                    'COUNT(DISTINCT CASE WHEN (o.status IS NULL OR o.status NOT IN (?, ?, ?)) AND d.finish_date IS NULL THEN o.id END) as without_deadline',
                    self::CLOSED_ORDER_STATUSES
                )
                ->first();
            ...
            $summary = [
                ...
                'active_orders' => (int) ($orderSummary->active_orders ?? 0),
                'completed_orders' => (int) ($orderSummary->completed_orders ?? 0),
                'canceled_orders' => (int) ($orderSummary->canceled_orders ?? 0),
                'overdue_orders' => (int) ($orderSummary->overdue_orders ?? 0),
                'due_today' => (int) ($orderSummary->due_today ?? 0),
                'due_next_7_days' => (int) ($orderSummary->due_next_7_days ?? 0),
            ...
            $factories = Factory::query()
                ...
                ->map(function (Factory $factory) use ($factoryStats) {
                    $stats = $factoryStats->get((int) $factory->id);
                    $active = (int) ($stats->active_orders ?? 0);
                    $overdue = (int) ($stats->overdue_orders ?? 0);
                    $unassigned = (int) ($stats->unassigned_orders ?? 0);
                    $awaiting = (int) ($stats->awaiting_admin_confirmation ?? 0);

                    return [
                        'id' => $factory->id,
                        'name' => $factory->name,
                        'value' => $factory->value,
                        'operators_count' => (int) $factory->operators_count,
                        'active_orders' => $active,
                        'overdue_orders' => $overdue,
                        'due_today' => (int) ($stats->due_today ?? 0),
                        'unassigned_orders' => $unassigned,
                        'awaiting_admin_confirmation' => $awaiting,
                        'completed_30d' => (int) ($stats->completed_30d ?? 0),
                        'health' => $this->factoryHealth($active, $overdue, $unassigned, $awaiting),
                    ];
                })
                ->values();
            ...
            $operators = User::query()
                ...
                ->map(function (User $operator) use ($operatorStats) {
                    $stats = $operatorStats->get((int) $operator->id);
                    $active = (int) ($stats->active_orders ?? 0);
                    $overdue = (int) ($stats->overdue_orders ?? 0);
            ...
            return response()->json([
                ...
                'filters' => [
                    ...
                    'creators' => User::query()->whereIn('id', $creatorIds)->orderBy('name')->get(['id', 'name']),
                    'order_statuses' => Order::query()->whereNotNull('status')->distinct()->orderBy('status')->pluck('status')->values(),
                ],
                'generated_at' => now()->toIso8601String(),
            ]);
        } catch (\Throwable $e) {
            Log::error('Admin operations dashboard failed', ['exception' => $e]);

            return response()->json(['message' => 'Unable to load admin dashboard.'], 500);
        }
    }
    ...
    public function orders(Request $request): JsonResponse
    {
        ...
        try {
            ...
            return response()->json(['message' => 'Unable to load filtered orders.'], 500);
        }
    }

    private function factoryStats($now, $today, $tomorrow, $thirtyDaysAgo)
    {
        [$openSql, $openBindings] = $this->openFactoryConditionSql('fo');
        ...
        return DB::table('factory_orders as fo')
            ...
            ->groupBy('fo.factory_id')
            ->select('fo.factory_id')
    ...
    private function operatorStats($now, $today, $tomorrow, $thirtyDaysAgo)
    {
        [$openSql, $openBindings] = $this->openFactoryConditionSql('fo');
    ...
    {
        if ($overdue >= 3 || $unassigned >= 5) return 'critical';
        if ($overdue > 0 || $unassigned >= 2 || $awaitingAdmin >= 3) return 'attention';
    ...
    {
        if ($overdue >= 2 || $active >= 8) return 'overloaded';
        if ($overdue >= 1 || $active >= 5) return 'busy';
    ...
    private function applyTimeRange(Builder $query, ?string $range): void
    {
        ...
        if ($range === 'today') {
            $query->whereHas('dates', fn (Builder $q) => $q->where('finish_date', '>=', $today)->where('finish_date', '<', $tomorrow));
            return;
        }

        if ($range === 'tomorrow') {
            $dayAfterTomorrow = (clone $tomorrow)->addDay();
            $query->whereHas('dates', fn (Builder $q) => $q->where('finish_date', '>=', $tomorrow)->where('finish_date', '<', $dayAfterTomorrow));
            return;
        }

        $days = $range === 'this_week' ? 7 - (int) $today->dayOfWeekIso : 7;
        $end = (clone $today)->addDays(max($days, 1))->endOfDay();
        $query->whereHas('dates', fn (Builder $q) => $q->where('finish_date', '>=', now())->where('finish_date', '<=', $end));
    }
}
